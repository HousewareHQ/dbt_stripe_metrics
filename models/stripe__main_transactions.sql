with balance_transaction as ( select * from {{ var('balance_transaction') }} ),

charge as ( select * from {{ var('charge')}} ),

customer as ( select * from {{ var('customer')}} ),

subscription as ( select * from {{ var('subscription') }} ),

invoice as ( select * from {{ var('invoice') }} ),

invoice_line_item as ( select * from {{ var('invoice_line_item') }} ),

plan as ( select * from {{ var('plan') }} ),

product as ( select * from {{ source('fivetran_stripe', 'product') }} ),

refund as ( select * from {{ var('refund')}} ),
...
invoice_details as ( select
    ...
    from invoice_line_item inner join invoice on invoice.invoice_id = invoice_line_item.invoice_id inner join plan on invoice_line_item.plan_id = plan.plan_id inner join product on plan.product_id = product.id ),

grouped_line_items as ( select
    invoice_id,
    ...
    from invoice_details group by 1, 2, 3, 4, 5 )
...
left join
    refund on
        balance_transaction.balance_transaction_id = refund.balance_transaction_id
left join
    charge on
        charge.balance_transaction_id = balance_transaction.balance_transaction_id
left join
    customer on charge.customer_id = customer.customer_id
left join
    grouped_line_items on charge.invoice_id = grouped_line_items.invoice_id
left join
    subscription on
        grouped_line_items.subscription_id = subscription.subscription_id
